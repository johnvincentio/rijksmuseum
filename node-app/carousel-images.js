...
//            console.log("getCollectionByPage; status code "+response.statusCode);
            ...
            json.artObjects.forEach(function(item) {
                if (item.webImage && item.webImage.url && item.webImage.width > item.webImage.height + 1500) {
                    if (APP.images.length < maxItems) {
                        APP.images.push({'objectNumber' : item.objectNumber, 'title' : item.longTitle, 'image' : item.webImage.url});
                    }
                }
            });
...
var allCollectionDone = function() {
    ...
    var writeStream = fs.createWriteStream('carousel.json');
